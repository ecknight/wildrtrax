...
- \code`wt_make_aru_tasks()` allow you to link media to tasks and to generate tags from recognizer or classifier results
...
my_projects <- wt_get_projects(sensor_id = 'ARU') |>
  ...
  mutate(data = purrr::map(\(project_id), wt_download_report(project_id = project_id, sensor_id = 'ARU', reports = 'main')))
```
