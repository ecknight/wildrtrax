...
download_and_load_rdata <- function(url, filename) {
  req <- request(url) |> 
    req_perform()
  writeBin(req$body, filename)
  load(filename, envir = .GlobalEnv)
  unlink(filename)  # Deletes the file from the disk
}

download_and_load_rdata(
  url = "https://raw.githubusercontent.com/ABbiodiversity/wildRtrax-assets/main/classifiers.RData",
  filename = "classifiers.RData"
)
...
## BirdNET
[BirdNET](https://birdnet.cornell.edu/) is a deep learning classifier that is trained to classify more than 6,000 of the world's most common bird species, including most North American bird species @kahl_2022. The model converts audio recordings into windows of spectrograms and outputs a probability score for each species in each one.
...
Visit the [BirdNET Github repository](https://github.com/birdnet-team/BirdNET-Analyzer) to run BirdNET on your own computer. The decision to pursue other applications should be made with the effect of BirdNET's low recall rate in mind:
...
The next WildTrax release will be accompanied by the introduction of a new deep learning model that can classify 306 of Canada's most common bird species. HawkEars will be implemented in the same fashion as BirdNET so that users will also be able to download a report and use the same set of `wildrtrax` functions demonstrated above on it. HawkEars is also freely available from [Github](https://www.github.com/jhuus/HawkEars).
...
dathawk <- curl::curl_download('https://raw.githubusercontent.com/ABbiodiversity/wildRtrax-assets/main/ExpertData_PR_Total_HawkEars.csv','ExpertData_PR_Total_HawkEars.csv') |>
  readr::read_csv() |>
  dplyr::filter(thresh >= 0.1) ### Required data

plot.p.h <- ggplot(dathawk) +
...
gridExtra::grid.arrange(plot.p.h, plot.r.h, plot.f.h, plot.pr.h, ncol=2,
